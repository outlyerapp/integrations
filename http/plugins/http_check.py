...
class HttpRequestPlugin(Plugin):
    ...
    def collect(self, _) -> Status:
        ...
        timeout = int(self.get('timeout', 10))
        ...
        try:
            ...
            if 'Content-Length' in response.headers:
                self.gauge('http.response_size', {'site': name}).set(float(response.headers['Content-Length']))
            elif content:
                self.gauge('http.response_size', {'site': name}).set(len(content))

            elapsed_time = response.elapsed.total_seconds()
            self.gauge('http.response_time', {'site': name}).set(elapsed_time)
            ...
            self.logger.info('URL %s is invalid and not found.', url)
            return Status.CRITICAL
        except requests.exceptions.ConnectTimeout:
            self.logger.info('Request timeout after %f seconds', timeout)
            return Status.CRITICAL
